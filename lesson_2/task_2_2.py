...
for item in range(len(temp_list)):
    if temp_list[item].isdigit():
        temp_list[item] = f'{int(temp_list[item]):02d}'
        new_list.append(temp_list[item])
    else:
        for item_str in temp_list[item]:
            if item_str == '+':
                temp_list[item] = f'+{int(temp_list[item]):02d}'
                new_list.append( temp_list[item])

# проверяем, что получилось то, что нужно
print(temp_list)
print(new_list)

for num in new_list:
       if num in temp_list:
        num_index = temp_list.index(num)
        temp_list.insert(num_index, '"')
        temp_list.insert(num_index + 2, '"')
